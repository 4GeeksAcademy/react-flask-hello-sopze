...
const authMessages= [
	"You're nobody, you not allowed here",
	"IDENTIFY YOURSELF KIDDO",
	"Hello Sir.Admin"
]
...
const Admin = () => {
	...
	React.useEffect(()=>{
		const _userCheck= async()=>{
			...
			set_authResult(await actions.checkAuth(1))
		}
		_userCheck()
	},[])
			...
			<div className="col-11 col-xl-6 mx-auto d-flex flex-column gap-4">
				{ authResult != null &&
					<section className="col-8 d-flex flex-column mx-auto px-1 py-3 bg-secondary-subtle rounded-4 border border-warning-subtle" >
						<h3 className="fw-bold fs-3 mb-5">{authMessages[authResult+1]}</h3>
						{ authResult == 1 &&
							<>
							<UserCardStupidWawa user={frontUser}/>
							<img className="mx-auto my-4"src={adminimage} style={{"width":"256px"}}/>
							</>
						}
